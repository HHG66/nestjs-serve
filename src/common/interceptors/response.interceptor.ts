  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return next.handle().pipe(
      tap((responseBody) => {
        ...
        // 根据状态码选择日志级别
        if (statusCode >= 500) {
          this.logger.error(`${logFormat} -auto - Body: ${JSON.stringify(body)} - Response: ${JSON.stringify(responseBody)}`, req);
        } else if (statusCode >= 400) {
          this.logger.warn(`${logFormat} -auto - Body: ${JSON.stringify(body)} - Response: ${JSON.stringify(responseBody)}`, req);
        } else {
          this.logger.log(`${logFormat} -auto - Body: ${JSON.stringify(body)} - Body: ${JSON.stringify(body)}  - Response: ${JSON.stringify(responseBody)}`, req);
        }
      }),
      map((data) => {
        // 转换响应数据为标准格式
